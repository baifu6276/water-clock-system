...
const {env}=require('./foundation-regression.cjs');
...
(async()=>{
  ...
  try {
    for(const scenario of scenarios){
      ...
      if(oldGas)for(const file of fs.readdirSync(path.join(__dirname,'../evidence/v44-sources')).filter(f=>f.endsWith('.gs'))){
        vm.runInContext(fs.readFileSync(path.join(__dirname,'../evidence/v44-sources',file),'utf8'),e.ctx,{filename:file});
      }
