...
const read = p => fs.readFileSync(path.join(root, p.replace(/^gas\//, 'sources/').replace(/Code\.gs$/, '程式碼.gs')), 'utf8');
...
let checks = 0;
function test(name, work) { if (module.parent) return; work(); checks++; console.log('PASS', name); }
...
test('V44 candidate GAS syntax and exact legacy dispatcher bytes', () => {
  files.forEach(f => new vm.Script(read('gas/'+f+'.gs')));
  assert.equal(read('gas/Code.gs'), fs.readFileSync(path.join(root,'evidence/v44-sources/程式碼.gs'),'utf8'));
});
...
test('complete explicit manifest preserves deployment settings and current-file scope', () => {
  ...
  const gs=fs.readdirSync(path.join(root,'sources')).filter(f=>f.endsWith('.gs')).map(f=>f==='程式碼.gs'?'Code.gs':f).sort();
...
if (!module.parent) console.log(`${checks} test groups passed; no network or production writes.`);
...
if (!module.parent) console.log(`${checks-statusStartCount} status test groups; ${checks} total foundation groups passed.`);
