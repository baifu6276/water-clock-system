...
T='release-candidates/gas-t4-control-no-flush/tests/'
def git(cwd,*args):return subprocess.check_output(['git',*args],cwd=cwd,text=True).strip()
assert git(REPO,'rev-parse','HEAD')=='5e272d981617d5940c88332aa7ba3bdead949b6a'
...
commands=[
 ...
 ('maintenance',[NODE,'--require','./'+T+'deny-network.cjs','--test','--test-reporter=tap',T+'maintenance.test.cjs'],REPO),
 ('wrapper',[NODE,'--require','./'+T+'deny-network.cjs','--test','--test-reporter=tap','experiments/t4-initial-prepare-wrapper/wrapper.test.cjs'],REPO),
