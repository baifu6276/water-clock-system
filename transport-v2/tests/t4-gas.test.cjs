...
const { makeEnv, legacyEnv, upgrade } = require('./foundation-fixture.cjs');
...
test('v3 historical rows remain readable and strict replay is read-only', () => {
  ...
  assert.equal(e.call('employeeLifecycleBaselineMigrate',p,'owner').success,true); const w=e.writes; upgrade(e);
...
test('v3 partial records remain readable but cannot be resumed as T4',()=>{
  ...
  e.fail('員工LINE綁定紀錄');e.call('employeeLifecycleBaselineMigrate',p,'owner');upgrade(e);const w=e.writes;
