...
window.onload = async function () {
  ...
  try {
    ...
    if (!liff.isInClient()) {
      throw new Error("請從 LINE App 開啟打卡系統");
    }
    ...
    showStatus("正在取得 LINE 身分...", "normal");
    const profile = await liff.getProfile();

    userId = profile.userId;
    userName = profile.displayName;

    document.getElementById("displayName").innerText = userName + "，辛苦了！";
    ...
    if (!result.success) {
      throw new Error(result.message || "無法讀取系統資料");
    }
    ...
    document.getElementById("employeeInfo").innerText =
      (employee.grade || "未設定級職") +
      " ｜ " +
      (employee.permission || "未設定權限");
    ...
    if (sites.length === 0) {
      throw new Error("目前沒有施工中的工地");
    }

    document.getElementById("siteSelect").disabled = false;
    document.getElementById("locationBtn").disabled = false;

    showStatus("✅ 身分確認成功，正在取得 GPS...", "success");
    await refreshLocation();
  } catch (error) {
    console.error(error);
    document.getElementById("displayName").innerText = "⚠️ 無法使用打卡系統";
